...
from ..datasets import (
    PerEpisodeStreamingHDF5DatasetFileHandler,
...
class StreamingRecorderManager(RecorderManager):
    def __init__(
        ...
        dataset_layout: str = "single_file",
    ) -> None:
        if dataset_layout not in {"single_file", "per_episode"}:
            raise ValueError(f"Unsupported dataset layout: {dataset_layout}")
        if dataset_layout == "per_episode" and env.num_envs != 1:
            raise ValueError("per_episode HDF5 recording currently requires num_envs=1")

        self.dataset_layout = dataset_layout
        cfg.dataset_file_handler_class_type = (
            PerEpisodeStreamingHDF5DatasetFileHandler
            if dataset_layout == "per_episode"
            else StreamingHDF5DatasetFileHandler
        )
        ...
        if cfg.dataset_export_mode == EnhanceDatasetExportMode.EXPORT_ALL_RESUME:
            if dataset_layout != "single_file":
                raise ValueError("--resume append mode is only valid for single_file datasets")
        ...
        # The base manager does not pass the Gym task ID to our streaming file
        # handler. Store it explicitly so inspection/replay tools can identify
        # the environment. This also fills missing metadata with --resume.
        if self._dataset_file_handler is not None and env_name:
            self._dataset_file_handler.add_env_args({"env_name": env_name, "type": 2})

        self._env_steps_record = torch.zeros(self._env.num_envs)
        self._episode_has_streamed_data = {env_id: False for env_id in range(self._env.num_envs)}
    ...
    def export_episodes(self, env_ids: Sequence[int] | None = None, from_step: bool = False) -> None:
        ...
        for env_id in env_ids:
            ...
            if episode is None:
                continue
            has_buffered_data = not episode.is_empty()
            has_streamed_data = self._episode_has_streamed_data.get(env_id, False)
            should_write = (
                has_buffered_data
                and (self._env_steps_record[env_id] >= self._flush_steps or not from_step)
            ) or (not from_step and has_streamed_data)
            if should_write:
                # NOTE: pre_export() is only available in IsaacLab 2.3.0+
                # TODO: remove this after we use IsaacLab 2.3.0+ by default
                if _AFTER_ISAACLAB_2_3_0 and has_buffered_data:
                ...
                if target_dataset_file_handler is not None:
                    ...
                    wrote_episode = target_dataset_file_handler.write_episode(episode, write_mode)
                    if wrote_episode and from_step:
                        self._episode_has_streamed_data[env_id] = True
                    elif wrote_episode:
                        self._episode_has_streamed_data[env_id] = False
                    self._clear_episode_cache([env_id])
                # Streaming APPEND chunks are not episode boundaries and must
                # not inflate the success/failure episode counters.
                if not from_step:
