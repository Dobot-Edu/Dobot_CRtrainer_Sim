...
def orange_grasped(
        ...
        gripper_joint_name: str | None = None) -> torch.Tensor:
    ...
    if gripper_joint_name is None:
        ...
        gripper_pos = robot.data.joint_pos[:, robot.data.joint_names.index(gripper_joint_name)]
        gripper_closed = gripper_pos > grasp_threshold
    grasped = torch.logical_and(pos_diff < diff_threshold, gripper_closed)
...
def put_orange_to_plate(
    ...
    if gripper_joint_name is None:
        ...
        gripper_pos = robot.data.joint_pos[:, robot.data.joint_names.index(gripper_joint_name)]
        gripper_open = gripper_pos < grasp_threshold
